...
SUITE["alg_dyn"] = BenchmarkGroup(["AlgDynam"])

for nesting in 6:-1:2
  SUITE["alg_dyn"]["nest_$nesting"] = BenchmarkGroup()

  SUITE["alg_dyn"]["nest_$nesting"]["generators"] = @benchmarkable generate_sys()
  diff_slw, diff_fst = generate_sys()
  SUITE["alg_dyn"]["nest_$nesting"]["construction"] = @benchmarkable combine_sys($diff_slw, $diff_fst, $nesting)
  system = combine_sys(diff_slw, diff_fst, nesting)
  SUITE["alg_dyn"]["nest_$nesting"]["init_sys"] = @benchmarkable initialize($nesting)
  state, coords = initialize(nesting)
  SUITE["alg_dyn"]["nest_$nesting"]["solving"] = @benchmarkable simulate($system, $state, $coords)
end
