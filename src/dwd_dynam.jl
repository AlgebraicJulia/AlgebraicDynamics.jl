...
"""
struct ContinuousDelayMachine{T} <: AbstractMachine{T}
    ninputs::Int
    nstates::Int
    noutputs::Int
    dynamics::Function
...
readout(f::ContinuousDelayMachine, u::AbstractVector, h = nothing, p = nothing, t = 0) = f.readout(u, h, p, t)
...
"""
eval_dynamics(f::ContinuousDelayMachine, u::AbstractVector, xs::AbstractVector, h::Function, p=nothing, t=0) = begin
    ninputs(f) == length(xs) || error("$xs must have length $(ninputs(f)) to set the exogenous variables.")
    f.dynamics(u, xs, h, p, t)
end
...
function oapply(d::WiringDiagram, ms::Vector{ContinuousDelayMachine{T}}) where T
    isempty(wires(d, input_id(d), output_id(d))) || error("d is not a valid composition syntax because it has pass wires")
    nboxes(d) == length(ms)  || error("there are $nboxes(d) boxes but $length(ms) machines")
    for box in 1:nboxes(d)
        fills(ms[box], d, box) || error("$ms[box] does not fill box $box")
    ...
    v = (u::AbstractVector, xs::AbstractVector, h::Function, p, t::Real) -> begin 
        ...
        reduce(vcat, map(enumerate(destruct(Inputs, readins))) do (i,x)
            eval_dynamics(ms[i], 
                          states[i], 
                          x, 
                          (p, t) -> collect(destruct(S, h(p,t))[i]), 
                          p, t)
        end)
    end
    ...
    return ContinuousDelayMachine{T}(length(input_ports(d)), length(apex(S)),
                   length(output_ports(d)), v, r)
end
