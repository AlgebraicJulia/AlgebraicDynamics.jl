...
tensor(ms::Vector{Monomial})::Monomial = sum(first.(ms)) => sum(last.(ms))
...
function DILS{T}(f::Function, ninput::Int, noutput::Int) where T
  DILS{T}(
    0=>ninput,  # input poly is ninput y^0
    0=>noutput, # input poly is nouput y^0
    0,          # trivial state
    (_, x) -> f(x),  #forward map
...
DILS{T}(c::AbstractVector) where T = DILS{T}(_ -> c, 0, length(c))
...
"""
function sequential_compose(d1::DILS{T}, d2::DILS{T}, braiding::Union{Nothing,AbstractVector} = nothing)::DILS{T} where T
  # @assert Base.isperm(braiding)
  #assert positions(output_poly(d1)) == positions(input_poly(d2)) # TODO: check directions match
  braiding = isnothing(braiding) ? (1:positions(output_poly(d1))) : braiding
  ...
  return DILS{T}(
    ...
    (u,x) -> forward_readout(d2, d2_state(u), d1_output(u,x)[braiding]),
    (u,x,y) -> backward_readout(d1, d1_state(u), x, d2_backward(u,x,y)[invperm(braiding)]),
    (u,x,y) -> vcat(eval_dynamics(d1, d1_state(u), x, d2_backward(u, x, y)[invperm(braiding)]), # updated state for d1
                    eval_dynamics(d2, d2_state(u), d1_output(u,x)[braiding], y))       # updated state for d2
  )
end


minus = DILS{Float64}(x->[x[1] - x[2]], 2, 1)

constant = DILS{Float64}([1,2])

one_minus_two = sequential_compose(constant, minus)
two_minus_one = sequential_compose(constant, minus, [2,1])
...
function tensor(d1::DILS{T}, d2::DILS{T}) where T
  d1_state = u -> u[1:d1.nstates]
  d2_state = u -> u[d1.nstates+1:end]
  DILS{T}(tensor(input_polys(d1), input_polys(d2)), 
          tensor(output_polys(d1), output_polys(d2)),
          d1.nstates + d2.nstates, 
          (u,x) -> vcat(forward_readout(d1, u[1:d1.nstates], x[1:first(input_poly(d1))], forward_readout(d2, )))
  )
end

"""TODO TEST"""
tensor(ds::Vector(DILS{T})) where {T} = reduce(tensor, ds, unit(DILS{T}}))
...
function oapply(d::WiringDiagram, dils::Vector{DILS})::DILS
  ...
  return reduce(compose, t_layers)
end
...
"""
function layerize(G::Graph)::Vector{Vector{Int}}
  seen = Set{Int}()
  layers = Vector{Int}[]
