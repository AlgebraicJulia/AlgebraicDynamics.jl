...
SUITE["alg_diffeq"] = BenchmarkGroup(["DiffEq"])

for nesting in 6:-1:2
  SUITE["alg_diffeq"]["nest_$nesting"] = BenchmarkGroup()
  SUITE["alg_diffeq"]["nest_$nesting"]["generators"] = @benchmarkable generate_sys($(2^nesting))
  dx, dy, x, y, α, t0, t1, u0 = generate_sys(2^nesting)
  SUITE["alg_diffeq"]["nest_$nesting"]["construction"] = @benchmarkable combine_sys($dx, $dy, $x, $y)
  Δ, Q = combine_sys(dx, dy, x, y)
  SUITE["alg_diffeq"]["nest_$nesting"]["init_sys"] = @benchmarkable initialize($Δ, $Q, $α, $t0, $t1, $u0)
  prob, alg = initialize(Δ, Q, α, t0, t1, u0)
  SUITE["alg_diffeq"]["nest_$nesting"]["solving"] = @benchmarkable simulate($prob, $alg)
end
