...
#Tests for discrete resource sharers
d = UWD(2)
add_box!(d, 2); add_box!(d, 2)
add_junctions!(d, 3)
set_junction!(d, [1,2,2,3])
set_junction!(d, [1,3], outer = true)

r = oapply(d, [DiscreteResourceSharer{Float64}(2, 2, x -> [x[1]*x[2], x[1]+x[2]],[1,2]), 
    DiscreteResourceSharer{Float64}(2, 3, x -> [x[1]+x[2], x[2]-x[1], x[3]], [1,2])])
@test nstates(r) == 4
@test nports(r) == 2
@test portmap(r) == [1,3]
@test eval_dynamics(r, [1,1,1,3]) == [1,3,0,3]
@test eval_dynamics(r, [1,3,0,3]) == [3,4,-3,3]
@test exposed_states(r, [1,3,0,3]) == [1,0]


d = UWD(0)
add_box!(d,2); add_box!(d,2); add_box!(d,1)
add_junctions!(d, 3)
set_junction!(d, [1,1,1,3,3])
r = oapply(d, [
  DiscreteResourceSharer{Float64}(2, 2, x -> [x[2], x[1]], [1,2]),
  DiscreteResourceSharer{Float64}(2, 2, x -> [x[1]+x[2], x[2]-x[1]],[1,2]),
  DiscreteResourceSharer{Float64}(1, 2, x -> [2*x[1], 2*x[1]*x[2]], [2])
])

@test nstates(r) == 4
@test nports(r) == 0
@test portmap(r) == []
@test eval_dynamics(r, [1,5,3,100]) == [6, 29, 6, 100]
